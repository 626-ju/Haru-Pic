    ...
    >
      <div className="bg-white p-8 rounded-2xl border-1 border-gray-300">
        <form onSubmit={handleSubmit} className="flex flex-col justify-start">
          <h2 className="text-xl text-black-500 font-bold">앨범 고유 아이디를 등록해주세요</h2>
          <input onChange={(e) => setAlbumKey(e.target.value)} className="mt-8 min-w-90 md:w-100 border-gray-300 border-1 rounded-xl px-6 py-4" placeholder="식별할 앨범 아이디를 입력해주세요" maxLength={20} required type="text" />
          <Button variant="submit" className='mt-6'>등록</Button>
        </form>
